...
@Component({
  ...
  templateUrl: './periscope-search.component.html',
  styleUrls: ['./periscope-search.component.css'],
    providers: []

})
export class PeriscopeSearchComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.http.get(Config.api + '/ml/periscope_categories', {headers:headers}, 'full')
            .subscribe(res => {
        ...
        this.http.get(Config.api+'/get_iList_names_periscope/' + this.currentUser.username+'/-id/')
            .subscribe(res => {
                // user_list = res.json();
    ...
    add_create_list() {
        ...
        let current_list = JSON.parse(localStorage.getItem('selected_list_peri'));
        for (let i in list) {
            if(list[i]) {
                ilist = true;
                // alert('jsdkada');
        ...
        if (current_list) {
            Swal.fire(
                ...
                mysvc.put(Config.api + '/create_add_ilist_periscope/', JSON.stringify({id: current_list.id, list: list}),
            ...
            Swal.fire({
                ...
                Swal.fire({
                    ...
                    mysvc.post(Config.api + '/create_add_ilist_periscope/', JSON.stringify({
                            name: result.value,
                            list: list,
                            username: currentUser.username
                        }),
                    ...
                    if (ilist) {
                        if(this.checklistname){
                            Swal.fire({
                                ...
                                mysvc.put(Config.api + '/create_add_ilist_periscope/', JSON.stringify({id: result, list: list}),
                                    {headers: headers}).map((response: Response) => response.json()).subscribe(
                                    data => {
                                        for (let i in this.inflist) {
                                            // this.inflist[i['id']] = e;
