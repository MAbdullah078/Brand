...
export class InstagramSearchComponent implements OnInit, AfterViewInit{
    ...
    ngOnInit() {
        ...
        this.http.get(Config.api + '/ml/instagram_categories', {headers:headers}, 'full')
            .subscribe(res => {
        ...
        this.http.get(Config.api+'/get_iList_names_in/' + this.currentUser.username+'/-id/')
            .subscribe(res => {
    ...
    get_default_insta_influencers(page: number) {
    ...
    add_create_list() {
        ...
        let current_list = JSON.parse(localStorage.getItem('selected_list_in'));
        for (let i in list) {
            if(list[i]) {
                ilist = true;
                // alert('jsdkada');
        ...
        if (current_list) {
            Swal.fire(
                ...
                mysvc.put(Config.api + '/create_add_ilist_in/', JSON.stringify({id: current_list.id, list: list}),
                ...
                if (dismiss === 'cancel') {
                    if (ilist) {
                        ...
                        Swal.fire({
                            ...
                            mysvc.put(Config.api + '/create_add_ilist_in/', JSON.stringify({id: result, list: list}),
                                {headers: headers}).map((response: Response) => response.json()).subscribe(
                                data => {
                                    for (let i in this.inflist) {
                                        // this.inflist[i['id']] = e;
