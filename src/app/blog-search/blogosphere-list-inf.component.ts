...
@Component({
    ...
    styleUrls: ['./blogosphere-list-inf.component.css']
})
export class BlogosphereListInfComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    saveEditable(t){
        ...
        this.httpNoPreloader.put(Config.api + '/update_name_ilist_dd/', JSON.stringify({id: this.searchQuery, name: t}),
            {headers: headers}).map((response: Response) => response.json()).subscribe(
            data => {
                localStorage.removeItem('selected_list_dd');
                localStorage.setItem('selected_list_dd',JSON.stringify({id:this.searchQuery, name:t}));
                this.list_name=t;
                this.loading = false;
    ...
    update_delete_list() {
        ...
        Swal.fire({
            ...
            mysvc.put(Config.api + '/update_delete_ilist_dd/', JSON.stringify({id: this.searchQuery, list: list}),
                {headers: headers}).map((response: Response) => response.json()).subscribe(
                data => {
                    this.setPage(1);
