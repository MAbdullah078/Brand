...
export class PaymentmethodsComponent implements OnInit, OnDestroy {
  ...
  constructor(  private formBuilder: FormBuilder, private _nav: Router, private serv: PaymentmethodsService, private router: Router, private route: ActivatedRoute,) {
    this.cardnumber = true;
    this.cardnumber2 = false;
    this.ccv = true;
    this.ccv2 = false;
    this.serv.getcounty().subscribe( data =>{
  ...
  zipcodeCheck(zipcode1) {
    if (zipcode1.length > 4) {
      this.endRequest = this.serv.zipcode(zipcode1).subscribe(
        data => {
