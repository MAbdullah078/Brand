...
@Component({
    ...
    templateUrl: './medium-list-inf.component.html',
    styleUrls: ['./medium-list-inf.component.scss']
})
export class MediumListInfComponent implements OnInit{
    pager: any= {};
    ptProfiles;
    infList: any = {};
    ...
    saveEditableList(t){
        ...
        this.httpNoPreloader.put(Config.api + '/update_name_ilist_md/', JSON.stringify({id: this.searchQuery, name: t}),
            ...
            data =>{
                ...
                localStorage.setItem('selected_list_med',JSON.stringify({id: this.searchQuery, name: t}));
                this.listName=t;
                this.loading= false;
            }
        )
    ...
    setPage(page: number) {
        ...
        this.sub = this.route.params.subscribe(params => {
            if (Number(params['query'])) {
                ...
                let headers = new Headers({'Authorization': 'JWT ' + currentUser.token});
                headers.append('Content-Type', 'application/json');
                this.http.get(Config.api + '/get_list_inf_md/' + params['query'] + '/?page='+page, {headers: headers}
                    , 'full')
                    .subscribe(res => {
    ...
    deleteSelected(){
        ...
        Swal.fire({
            ...
            cancelButtonText: 'Cancel'
                ...
                let headers= new Headers({'Authorization': 'JWT ' + currentUser['token']});
                headers.append('Content-Type', 'application/json');
                mysvc.put(Config.api + '/update_delete_ilist_md/' , JSON.stringify({id: this.searchQuery, list: list}),
                    {headers: headers}).map((response: Response) => response.json()).subscribe(
                    data=>{
                        this.setPage(1);
